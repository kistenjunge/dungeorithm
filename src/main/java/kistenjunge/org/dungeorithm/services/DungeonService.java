...
/**
 * 
 * All Dungeons are square with an odd number of rows and columns. The
 * Rooms have odd numbers in height and width.
 * 
 */
public class DungeonService
{
  ...
  private boolean checkIfRoomFits(Room room)
  {
    ...
    for (Coordinate roomCoord : roomCoords)
    {
      ...
      {
        if (dungeonTile != TileType.PERIMETER && dungeonTile != TileType.DOOR)
        {
          return false;
        }
      }
  ...
  /**
   * sub get_opts { my $opts = { 'seed' => time(), 'n_rows' => 31, #
   * must be an odd number 'n_cols' => 31, # must be an odd number
   * 'dungeon_layout' => 'None', 'room_min' => 3, # minimum room size
   * 'room_max' => 9, # maximum room size 'room_layout' =>
   * 'Scattered', # Packed, Scattered 'corridor_layout' => 'Bent',
   * 'remove_deadends' => 50, # percentage 'add_stairs' => 2, # number
   * of stairs 'map_style' => 'Standard', 'cell_size' => 18, # pixels
   * }; return $opts; }
   * 
